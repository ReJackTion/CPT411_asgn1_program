...
function Item(props: ItemProps) {
  return (
    <a className={`p-2 ${props.className}`} href={props.destino}>
      {props.nome}
    </a>
  );
}
  ...
  return (
    <div className="duration-1000 delay-1000 transition-transform">
      <Item
        ...
        destino={"/#savedtexts"}
      />
      <Item className={props.className} nome={`About`} destino={"/#about"} />
      {props.StatusDarkMode ? (
        <button
          className="p-2 uppercase text-white"
          onClick={() => props.StatusDarkMode(props.statusDM)}
        >
          Dark Theme:
          <small className="font-bold px-1 rounded-none">
            {props.statusDM ? "on" : "off"}
          </small>
        </button>
      ) : (
        <></>
...
export default function Navbar(props: NavBarProps) {
  ...
  return (
    <div
      className={`fixed w-screen p-3
                        ${darkModeNavbar}    `}
    >
      {ham ? (
        <div className="flex justify-end">
          <button
            onClick={() => setItensHam(!itensHam)}
        ...
        <div className="flex justify-center uppercase text-white font-bold">
          <ItensNavbar
            StatusDarkMode={props.StatusDarkMode}
